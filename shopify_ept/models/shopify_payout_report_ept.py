...
class ShopifyPaymentReportEpt(models.Model):
    ...
    def get_paid_move_line_amount(self, statement_line, paid_invoices):
        ...
        if statement_line.refund_invoice_id:
            ...
            paid_move_lines = payment_id.move_id.line_ids.filtered(lambda x: x.credit != 0.0)
            if not paid_move_lines:
                paid_move_lines = payment_id.invoice_ids.line_ids.filtered(
                    lambda x: x.credit != 0.0 and x.account_type == 'asset_receivable')
        else:
            payment_id = paid_invoices.line_ids.matched_credit_ids.credit_move_id.payment_id
            paid_move_lines = payment_id.move_id.line_ids.filtered(lambda x: x.debit != 0.0)
            if not paid_move_lines:
                paid_move_lines = payment_id.invoice_ids.line_ids.filtered(
                    lambda x: x.debit != 0.0 and x.account_type == 'asset_receivable')
