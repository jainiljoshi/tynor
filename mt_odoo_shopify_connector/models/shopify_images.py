...
class ShopifyProductImage(models.Model):
    ...
    @api.model
    def cron_download_shopify_images(self):
        # Retrieve up to 200 images that don't have binary data yet
        pending_images = self.search([('shopify_image', '=', False), ('url', '!=', False)], limit=200)
        import base64
        import requests
        for img in pending_images:
            try:
                if self.get_extension(img.url) != '.webp':
                    response = requests.get(img.url, timeout=10)
                    if response.status_code == 200:
                        image_data = base64.b64encode(response.content)
                        img.write({'shopify_image': image_data})
                        # If this is the main image, sync to product.template
                        if img.is_main_image and img.product_template_id:
                            img.product_template_id.write({'image_1920': image_data})
                self.env.cr.commit()
            except Exception as e:
                _logger.warning("Failed to download image %s: %s", img.url, str(e))
