...
class AccountFiscalPosition(models.Model):
    ...
    def _get_fiscal_position(self, partner, delivery=None):
        ...
        # If no connector context, just use native Odoo logic entirely
        if not origin_country_id:
        ...
        # Read property directly without triggering _compute_fiscal_position_id
        IrProperty = self.env['ir.property'].sudo()
        manual_fiscal_position = (
            IrProperty._get('property_account_position_id', 'res.partner', delivery.id)
            or IrProperty._get('property_account_position_id', 'res.partner', partner.id)
        )
        if manual_fiscal_position:
            return manual_fiscal_position
